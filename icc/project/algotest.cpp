...
{	
	...
	for (size_t i(0); i < size_list-1; ++i)
	{
		...
		{
			if (not one_before_two(list[i].movements, list[j].movements))
			{
				string temp(list[i].movements);
				list[i].movements = list[j].movements;
				list[j].movements = temp;
			}
		}
	}
